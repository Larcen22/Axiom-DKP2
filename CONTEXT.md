...
- **Render strategy:** Overview recent loot shows first 15 dated rows; item table caps at 100 rendered rows; standings/loot/roster/raids use **client-side pagination** (25 / 25 / 25 / 5 rows per page respectively) with a shared pager component.
...
Constants: `RECENT_LOOT_COUNT = 15`, `ITEMS_RENDER_CAP = 100`, `STANDINGS_PAGE_SIZE = 25`, `LOOT_PAGE_SIZE = 25`, `ROSTER_PAGE_SIZE = 25`, `MEMBER_LOOT_CAP = 50`, `RAID_PAGE_SIZE = 5`. Module-level state: `db = { users, loot, items, roster }` (set in `init()`, used by `openMember()`).
