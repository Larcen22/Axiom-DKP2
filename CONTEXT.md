...
- **Topbar data-as-of:** `#data-asof` shows “Data through YYYY-MM-DD” — the newest raid date in the export (`setDataAsOf`, string max over normalized dates) so officers can tell at a glance how fresh the data is; empty when there are no dated raids. Hidden on mobile with the rest of `.topbar-meta .meta-text`.
