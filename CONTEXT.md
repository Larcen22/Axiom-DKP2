...
| `loadLoot()` | Normalized rows `{ date, player, user, item, raid, raidId, dkpSpent }` — `date`/`raid` are null when unresolvable; `user` is the owner username_id (string or null) for member-level grouping; `raidId` is the raid's `raid_id` normalized to a string (kept even when unknown) for exact joins. All dates normalized to `YYYY-MM-DD` via `isoDate()` (exports occasionally carry full ISO timestamps; empty/unparseable values become null so junk can never reach a date cell). |
| `loadRaids()` | `[{ id, date, name, dkpValue, attendees:string[], attendeeUserIds:string[] }]` — dates likewise normalized to `YYYY-MM-DD`. `id` is the `raid_id` normalized to a string; raid **names repeat over time** (multi-raid days), so per-raid joins must use `id`, never `name`. |
...
- **Member Detail:** Drill-down via `.member-link` (every render site carries `data-return` = origin view: overview / standings / roster / raid). The back button returns to that origin (`memberReturnView`, same pattern as Raid Detail) and re-activates the matching nav item. Layout: header (name + rank badge — roster `Rank`, hidden when the default "member") with character chips beneath, a 3-card DKP row, a Raid Attendance panel, then Loot History. DKP prefers `users.json` account values, falling back to roster sums. Characters come from the roster (matched by member name). Member loot is scoped by **character name** (case-insensitive) — not username_id — and paginated (`MEMBER_LOOT_PAGE_SIZE = 10`) with the same shared pager as the other views.
- **Raid Detail:** Drill-down via `.raid-link` on any raid name (Recent Raids table + Raid History rows; each link carries `data-id` = `raid_id` and `data-return` = origin view). Layout mirrors Member Detail: header (name + date pill), 3-card stat row (Members Attended / Items Awarded / DKP Spent), an Attendees panel, then a paginated Loot table (`RAID_LOOT_PAGE_SIZE = 10`, biggest award first) whose Owner column links to the member profile. All data is scoped to the exact raid via `raid_id` — never by name (names repeat over time). Attendees are grouped into members through roster character-name matching and rendered as compact wrapping chips (`.attendee-chips`): one chip per member (name clickable → profile), characters listed inside a chip only when that member multi-boxed, unrostered characters as dashed muted chips; the status line reports `N members · M characters attended (K not in roster)`. All attendees count toward the Members stat. The back button returns to the origin view (`raidReturnView`, set from the clicked link's `data-return`) and re-activates that nav item — same pattern as Member Detail.
