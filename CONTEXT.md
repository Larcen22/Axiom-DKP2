...
- **Command palette:** Ctrl+K / "/" (outside inputs) / topbar button opens an overlay searching members, characters, items (loot names), raids, plus view quick-jumps when the query is empty. Enter/click navigates — member/char rows open Member Detail and raid rows open Raid Detail with the current view as `data-return` origin (so ← Back works); item rows open pqdi.cc in a new tab; view rows switch nav views.
