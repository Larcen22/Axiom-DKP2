...
|---|---|---|
| `index.html` | 277 lines | App shell: top nav, sidebar navigation, 7 view sections (Overview / Items / Standings / Loot / Roster / Member / Raids), footer. Loads PapaParse from CDN + local scripts. |
| `js/data.js` | 217 lines | **Data layer** — fetches/validates/normalizes all local data files; builds lookup indexes. Exposes global `Data` object. |
| `js/app.js` | 622 lines | **Presentation layer** — sidebar nav switching, renders the 7 views from data, pagination for standings/loot/roster/raids, member drill-down (`openMember`), debounced item search. |
...
- **Overview stats:** `renderOverview()` computes a 7-day and a 30-day `YYYY-MM-DD` cutoff (today − N) and string-compares them against resolved dates. **Items Awarded** = loot rows dated in the 7-day window; **Active Raiders** = unique attendee `username_id`s across `raids.json` raids dated in the 7-day window (counted by owner, so a member on multiple characters counts once — attendance is the only per-raid membership signal, so it comes from raids, not users); **Avg Raid Size · past 30d** = average attendee count over raids dated in the 30-day window; **Applicants** (`#stat-new-members`) = unique roster members whose application date falls in the 30-day window.
...
| `loadRaids()` | Loads `raids.json` fully for the Raid History view + Overview stats: `[{ date, name, dkpValue, attendees: string[], attendeeUserIds: string[] }]` (`attendees` = character names; `attendeeUserIds` = owner ids, used by the Active Raiders stat). |
...
| `renderOverview(users, loot, items, raids, roster)` | Fills 6 stat cards across two rows: **Total DKP Available** (Σ `activeDkp`), **Items Awarded · past week** (loot rows dated within 7 days of today), **Top Spender** (max `spent` → username), **Active Raiders** (unique attendee `username_id`s from raids dated within 7 days of today — counted by owner, multi-character members count once), **Avg Raid Size · past 30d** (`#stat-avg-raid-size`, average attendees per raid over the last 30 days, rounded; "–" if no raids in window), **Applicants** (`#stat-new-members`, unique roster members whose application date falls within 30 days of today). Then renders the **Recent Loot** table: all loot dated within the past 7 days (same window as the Items Awarded stat), newest first, paginated 25/page via `renderRecentLootPage()` / `renderRecentLootPager()` (delegated on `#recent-loot-pager`); columns Date / Player / Item / **Raid** / DKP Spent; item names linked via `Data.itemLink`. Status line goes to `#recent-loot-status` (NOT `#loot-status`, which belongs to the Loot History view). |
...
| `renderRaids(raids)` / `renderRaidsPage(page)` | Full raid log, newest first (undated last); 5/page. Columns: Date / Raid / DKP / Raiders (comma-joined attendee names) / Total (attendee count). |
...
- **`#overview`:** two `.stat-grid` rows — row 1: stat cards `#stat-total-dkp`, `#stat-items-awarded`, `#stat-top-earner` (label reads "Top Spender"), `#stat-raiders`; row 2: `#stat-avg-raid-size` ("Avg Raid Size · past 30d"), `#stat-new-members` (label reads "Applicants") + two empty `.stat-spacer` divs so the auto-fit grid keeps the same item count and card widths as row 1. Then `#recent-loot-status` (renamed from `#loot-status` to fix a duplicate-id collision with the Loot History view); `#recent-loot-table` (Date / Player / Item / Raid / DKP Spent); `#recent-loot-pager` (25/page, delegated click).
...
- **`#raids`:** `#raids-status`; `#raids-table` (Date / Raid / DKP / Raiders / Total); `#raids-pager`.
